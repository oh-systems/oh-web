...
export default function Home() {
  ...
  useLayoutEffect(() => {
    ...
    const updateAnimationProgress = () => {
      ...
      // Reset timing on first frame after initial load completes
      if (!animationStarted) {
      ...
      if (isAutoPlaying && !isManualNavigationRef.current) {
        ...
        const autoScrollProgress = (autoPlayTime / 70) * maxScrollRange; // 70 seconds total animation (removed 10s delay)
      ...
      if (manualProgressRef.current !== null) {
        ...
        autoPlayTime = manualProgressRef.current * 70;
      ...
      // Start animation immediately when swap begins for smooth transition
      let castAnimationProgress = 0;
      if (rawProgress >= MODEL_SWAP_CONFIG.swapStart) {
        // Cast Shadows animation starts with the swap for smooth transition
        const castStart = MODEL_SWAP_CONFIG.swapStart; // Start at swap beginning (0.286)
        ...
        {initialLoadComplete && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-full h-full flex items-center justify-center">
              {/* Initial Scroll sequence - fade out during swap */}
              {castSwapProgress < 1 && initialScrollProgress > 0.01 && (
                <div
                  style={{
                    position: "absolute",
                    opacity: Math.max(0, 1 - castSwapProgress * 2), // Fade out during first half of swap
              ...
              {/* Cast Shadows sequence - pre-mount early for background loading, visible only during swap */}
              {animationProgress >= 0.1 && laptopSwapProgress < 1 && (
                <div
                  style={{
                    position: "absolute",
                    inset: 0,
                    width: "100vw",
                    height: "100vh",
                    opacity: castSwapProgress > 0 ? Math.min(1, castSwapProgress * 2) : 0, // Invisible until swap, then fade in
                    pointerEvents: castSwapProgress < 0.5 ? "none" : "auto",
                    visibility: castSwapProgress > 0 ? "visible" : "hidden", // Hide from render tree when not in swap
                  }}
                >
                  <CastShadowsSequence
                    scrollProgress={castAnimationProgress}
                    priority={castSwapProgress > 0}
                  />
                </div>
              )}

              {/* Third Laptop sequence - pre-mount and crossfade */}
              {laptopSwapProgress > 0 && (
                <div
                  style={{
                    display: "flex",
                    position: "absolute",
                    ...
                    opacity: Math.min(1, laptopSwapProgress * 2), // Fade in during first half of swap
