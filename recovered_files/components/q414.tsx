  ...
  useLayoutEffect(() => {
    ...
    const updateAnimationProgress = () => {
      ...
      if (rawProgress >= MODEL_SWAP_CONFIG.swapStart) {
        ...
        // Debug logging during swap
        if (swapProgress > 0 && swapProgress < 1 && Math.random() < 0.1) {
          console.log(`🔄 Cast swap progress: ${swapProgress.toFixed(3)}, rawProgress: ${rawProgress.toFixed(3)}`);
        }
      } else {
      ...
      if (rawProgress >= MODEL_SWAP_CONFIG.animationStart) {
        ...
        // Debug logging
        if (castAnimationProgress < 0.01 && Math.random() < 0.2) {
          console.log(`🎬 Cast animation progress: ${castAnimationProgress.toFixed(4)}, rawProgress: ${rawProgress.toFixed(3)}`);
        }
      }
        ...
        {initialLoadComplete && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-full h-full flex items-center justify-center">
              {/* Initial Scroll sequence - conditionally mounted with fade out */}
              {castSwapProgress < 1 && initialScrollProgress > 0.01 && (
                <div
                  style={{
                    position: "absolute",
                    opacity: Math.max(0, 1 - castSwapProgress), // Fade out linearly
                    pointerEvents: castSwapProgress > 0.9 ? "none" : "auto",
                  }}
                >
                  <InitialScrollSequence
                    width={1000}
              ...
              {/* Cast Shadows sequence - mount early for preload, fade in during swap */}
              {animationProgress >= 0.25 && laptopSwapProgress <= 0.5 && (
                <div
                  style={{
                    position: "absolute",
                    inset: 0,
                    width: "100vw",
                    height: "100vh",
                    opacity: Math.min(1, castSwapProgress), // Fade in linearly with swap
                    pointerEvents: castSwapProgress < 0.1 ? "none" : "auto",
                  }}
                >
                  <CastShadowsSequence
              ...
              {/* Third Laptop sequence - only mount when active */}
              {laptopSwapProgress >= 0.5 && (
                <div
                  style={{
                    display: "flex",
                    position: "absolute",
