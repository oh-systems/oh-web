...
export default function Home() {
  ...
  const handleSectionClick = (section: 'overview' | 'mission' | 'space') => {
    ...
    }, 2000); // 2 second pause to allow user to see the section
  };
  ...
  const MODEL_SWAP_CONFIG = {
    ...
    swapEnd: 0.287, // Instant transition - 0.1% crossfade into Cast Shadows
    animationStart: 0.287, // Cast Shadows animation begins immediately
  ...
  useLayoutEffect(() => {
    ...
    const updateAnimationProgress = () => {
      ...
      if (isManualNavigationRef.current && manualProgressRef.current !== null) {
        effectiveProgress = manualProgressRef.current;
        // Sync the scroll accumulator to the manual progress to avoid jumps
        scrollAccumulator = effectiveProgress * maxScrollRange;
        // Also sync autoPlayTime to avoid auto-play jumping
        autoPlayTime = (effectiveProgress * maxScrollRange / maxScrollRange) * 70;
      } else {
        effectiveProgress = scrollAccumulator / maxScrollRange;
      }
    ...
    TEXT_SEQUENCE.physicalIntegrationsStart,
    TEXT_SEQUENCE.physicalIntegrationsEnd,
  ]);
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-full h-full flex items-center justify-center">
                ...
                style={{
                  display:
                    castSwapProgress > 0.5 || initialScrollProgress <= 0.01
                      ? "none"
                      : "block", // Hide if transitioning or at start
                  position: "absolute",
                  pointerEvents:
                    castSwapProgress > 0.5 || initialScrollProgress <= 0.01
                      ? "none"
              ...
              <div
                style={{
                  display:
                    castSwapProgress < 0.6 || laptopSwapProgress > 0.4
                      ? "none"
                      : "block",
                  position: "absolute",
