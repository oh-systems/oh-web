...
import { useScrollSpeedLimiter } from "../../hooks";
...
export default function Home() {
  ...
  const scrollLimiter = useScrollSpeedLimiter({
    ...scrollSpeedConfig,
    onScrollUpdate: (progress: number, velocity: number, isHighVelocity: boolean) => {
  ...
  useLayoutEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Process wheel event with velocity limiting
      const { limitedDelta, velocity, isHighVelocity } = scrollLimiter.processWheelEvent(e);

      // Use limited delta instead of raw deltaY
      scrollAccumulator += limitedDelta;
      ...
      // Adaptive auto-play resume delay based on scroll velocity
      const resumeDelay = isHighVelocity ? 4000 : velocity > 400 ? 3000 : 2000;
      setTimeout(() => {
        isAutoPlaying = true;
    ...
    // Handle touch events for mobile/trackpad with speed limiting
    let touchStartY = 0;
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      const deltaTime = Math.max(16, currentTime - touchStartTime); // Min 16ms

      // Manual touch takes over from auto-play
      isAutoPlaying = false;

      // Process touch with velocity limiting
      const { limitedDelta, velocity, isHighVelocity } = scrollLimiter.processTouchEvent(deltaY, deltaTime);
      ...
      // Adaptive auto-play resume delay for touch
      const resumeDelay = isHighVelocity ? 3500 : velocity > 300 ? 2500 : 2000;
      setTimeout(() => {
        isAutoPlaying = true;
        lastAutoPlayTime = performance.now();
      }, resumeDelay);
